set_property IOSTANDARD LVCMOS33 [get_ports {show[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports {show[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {show[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {show[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {show[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {show[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {show[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_ctrl[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_ctrl[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_ctrl[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd_ctrl[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports clk]
set_property IOSTANDARD LVCMOS33 [get_ports content_ctrl]
set_property IOSTANDARD LVCMOS33 [get_ports press]
set_property IOSTANDARD LVCMOS33 [get_ports rst_before]


set_property IOSTANDARD LVCMOS33 [get_ports am_or_not]
set_property PACKAGE_PIN W7 [get_ports {show[6]}]
set_property PACKAGE_PIN W6 [get_ports {show[5]}]
set_property PACKAGE_PIN U8 [get_ports {show[4]}]
set_property PACKAGE_PIN V8 [get_ports {show[3]}]
set_property PACKAGE_PIN U5 [get_ports {show[2]}]
set_property PACKAGE_PIN V5 [get_ports {show[1]}]
set_property PACKAGE_PIN U7 [get_ports {show[0]}]
set_property PACKAGE_PIN V7 [get_ports am_or_not]
set_property PACKAGE_PIN W4 [get_ports {ssd_ctrl[3]}]
set_property PACKAGE_PIN V4 [get_ports {ssd_ctrl[2]}]
set_property PACKAGE_PIN U4 [get_ports {ssd_ctrl[1]}]
set_property PACKAGE_PIN U2 [get_ports {ssd_ctrl[0]}]
set_property PACKAGE_PIN W5 [get_ports clk]
set_property PACKAGE_PIN V17 [get_ports content_ctrl]
set_property PACKAGE_PIN T17 [get_ports press]
set_property PACKAGE_PIN W19 [get_ports rst_before]

set_property IOSTANDARD LVCMOS33 [get_ports {led[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {led[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {led[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {led[0]}]
set_property PACKAGE_PIN L1 [get_ports {led[3]}]
set_property PACKAGE_PIN P1 [get_ports {led[2]}]
set_property PACKAGE_PIN N3 [get_ports {led[1]}]
set_property PACKAGE_PIN P3 [get_ports {led[0]}]

set_property IOSTANDARD LVCMOS33 [get_ports {ledc[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ledc[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ledc[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ledc[0]}]
set_property PACKAGE_PIN V19 [get_ports {ledc[3]}]
set_property PACKAGE_PIN U19 [get_ports {ledc[2]}]
set_property PACKAGE_PIN E19 [get_ports {ledc[1]}]
set_property PACKAGE_PIN U16 [get_ports {ledc[0]}]
